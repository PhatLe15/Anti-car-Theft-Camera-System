...
char *cmdMail[] = { "/usr/bin/mail", "-s","Breaking Alert!!!","rasppi24@gmail.com", 0 };

void runpipe(int pfd[]);  //pass command and rund the pipe

void sendEmail(char *detectAt/*, char *optionsEcho, char *optionsMail*/) {
        cmdEcho[1] = detectAt; //pass content of the mail
        int pid, status;
        int fd[2];

        pipe(fd); //create a pipe to pass output from echo to mail command
