...
#Setup GPIO pin for servo, may conflict with C program.
...
def my_stuff(image_frame, xy_pos):
    ...
    """
    #Setup PWM process
    servo_pin = 17 #Set pin for servo control
    ...
    if x_pos < IMAGE_W/4:
        if serv == 12:
            ...
            serv = serv + 1.0
        pwm.ChangeDutyCycle(serv)
        time.sleep(0.5)
    elif x_pos > IMAGE_W - IMAGE_W/4:
        if serv == 0:
            serv = 0
        else:
            serv = serv - 1.0
...
if __name__ == '__main__':
    while True:
        ...
        try:
            ...
            pwm = GPIO.PWM(servo_pin, 50)
            pwm.start(7)
            pwm.ChangeDutyCycle(7.0) #90 degrees, use c in future
            time.sleep(0.5)
            pwm.ChangeDutyCycle(0)
            pwm.stop()
            GPIO.cleanup()
